...
def random_rotate(img, mask, angle=180):
    with tf.name_scope("random_rotate"):
        angle = tf.random_uniform([1], minval=-angle, maxval=angle, seed=42)
...
def random_rot90(img, mask, prob=0.5):
    with tf.name_scope("random_ro90"):
...
def random_brightness(img, mask, max_delta=0.5):
    with tf.name_scope("random_brightness"):
        img = tf.image.random_brightness(img, max_delta=max_delta, seed=42)
...
def random_contrast(img, mask, min=0., max=1.):
    with tf.name_scope("random_contrast"):
        img = tf.image.random_contrast(img, lower=min, upper=max, seed=42)
