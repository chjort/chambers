...
def rmac_regions(W, H, L):
    ...
    for l in range(1, L + 1):
        ...
        for i_ in cenH:
            for j_ in cenW:
                R = np.array([j_, i_, wl, wl], dtype=np.int)  # (W_offset, H_offset, W, H)
                if not min(R[2:]):
                    continue
...
class RMAC(tf.keras.layers.Layer):
    ...
    def call(self, x, mask=None):
        def batch_pool_rois(roi_box):
            ...
            tw = roi_box[2]
            th = roi_box[3]
            rois = tf.image.crop_to_bounding_box(x, offset_height=oh, offset_width=ow,
                                                 target_height=th, target_width=tw)
            pooled_roi = self.pool_roi(rois)  # [batch_size, n_channels]
            return pooled_roi

        pooled_roi_batch = K.map_fn(batch_pool_rois, self.roi_boxes,
                                    dtype=tf.float32)  # [n_rois, batch_size, n_channels]
        pooled_roi_batch = tf.transpose(pooled_roi_batch, [1, 0, 2])  # [batch_size, n_rois, n_channels]
