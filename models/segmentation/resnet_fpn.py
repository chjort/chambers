...
def conv_bn_relu(input, num_channel, kernel_size, stride, name, padding='same', bn_axis=-1, bn_momentum=0.99,
    ...
    x = layers.Conv2D(filters=num_channel, kernel_size=(kernel_size, kernel_size),
                      strides=stride, padding=padding,
                      kernel_initializer="he_normal",
                      use_bias=use_bias,
                      name=name + "_conv")(input)
    x = layers.BatchNormalization(name=name + '_bn', scale=bn_scale, axis=bn_axis, momentum=bn_momentum,
                                  epsilon=1.001e-5, )(x)
...
def conv_relu(input, num_channel, kernel_size, stride, name, padding='same', use_bias=True, activation='relu'):
    x = layers.Conv2D(filters=num_channel, kernel_size=(kernel_size, kernel_size),
                      ...
                      use_bias=use_bias,
                      name=name + "_conv")(input)
    x = layers.Activation(activation, name=name + '_relu')(x)
    return x
...
def decoder_block(input, filters, skip, block_name):
    ...
    x = layers.concatenate([x, skip], axis=-1, name=block_name + '_concat')
...
def create_pyramid_features(C1, C2, C3, C4, C5, feature_size=256):
    P5 = layers.Conv2D(feature_size, kernel_size=1, strides=1, padding='same', name='P5',
                       kernel_initializer="he_normal")(C5)
    P5_upsampled = layers.UpSampling2D(name='P5_upsampled')(P5)

    P4 = layers.Conv2D(feature_size, kernel_size=1, strides=1, padding='same', name='C4_reduced',
                       kernel_initializer="he_normal")(C4)
    P4 = layers.Add(name='P4_merged')([P5_upsampled, P4])
    P4 = layers.Conv2D(feature_size, kernel_size=3, strides=1, padding='same', name='P4',
                       kernel_initializer="he_normal")(P4)
    P4_upsampled = layers.UpSampling2D(name='P4_upsampled')(P4)

    P3 = layers.Conv2D(feature_size, kernel_size=1, strides=1, padding='same', name='C3_reduced',
                       kernel_initializer="he_normal")(C3)
    P3 = layers.Add(name='P3_merged')([P4_upsampled, P3])
    P3 = layers.Conv2D(feature_size, kernel_size=3, strides=1, padding='same', name='P3',
                       kernel_initializer="he_normal")(P3)
    P3_upsampled = layers.UpSampling2D(name='P3_upsampled')(P3)

    P2 = layers.Conv2D(feature_size, kernel_size=1, strides=1, padding='same', name='C2_reduced',
                       kernel_initializer="he_normal")(C2)
    P2 = layers.Add(name='P2_merged')([P3_upsampled, P2])
    P2 = layers.Conv2D(feature_size, kernel_size=3, strides=1, padding='same', name='P2',
                       kernel_initializer="he_normal")(P2)
    P2_upsampled = layers.UpSampling2D(size=(2, 2), name='P2_upsampled')(P2)

    P1 = layers.Conv2D(feature_size, kernel_size=1, strides=1, padding='same', name='C1_reduced',
                       kernel_initializer="he_normal")(C1)
    P1 = layers.Add(name='P1_merged')([P2_upsampled, P1])
    P1 = layers.Conv2D(feature_size, kernel_size=3, strides=1, padding='same', name='P1',
...
def prediction_fpn_block(x, name, upsample=None):
    ...
    if upsample:
        x = layers.UpSampling2D(upsample)(x)
    return x
