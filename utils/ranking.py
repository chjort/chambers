...
def score_matrix_to_binary_ranking(similarity_matrix, query_labels, candidate_labels, remove_top1=False):
    query_labels = tf.reshape(query_labels, [-1, 1])
    candidate_labels = tf.reshape(candidate_labels, [-1, 1])
    pair_signs = tf.cast(tf.equal(query_labels, tf.transpose(candidate_labels)), tf.float32)
